...
class PdoGsb {
    ...
    /**
     * Indique que le frais hors forfait  dont l'id est passé en argument est rejeté (ajout au libellé)
     *
     * @param String $idFrais ID du frais
     *
     * @return null
    ...
    public function rejeterFraisHorsForfait($idFrais) {
        ...
        if (substr($libelle, 0, 9) != 'REFUSE - ') {
            ...
            $requetePrepare->bindParam(':unIdFrais', $idFrais, PDO::PARAM_STR);
            $requetePrepare->bindParam(':nouveauLibelle', $libelle, PDO::PARAM_STR);
            $requetePrepare->execute();
        }
    }
    ...
    public function reporterFraisHorsForfait($idFrais) {
        $leMoisSuivant = PdoGsb::getMoisSuivantFraisHF($idFrais);
        ajouterErreur($leMoisSuivant);
    ...
    public function getMoisSuivantFraisHF($idFrais) {
        $requetePrepare = PdoGsb::$monPdo->prepare(
                'SELECT lignefraishorsforfait.mois AS leMois '
                . 'FROM lignefraishorsforfait '
    ...
    /**
     * Retourne l'ensemble des visiteurs (id nom et prénom)
     * 
     * @return l'id, le nom et le prénom sous la forme d'un tableau associatif
     */
    ...
    /**
     * indique si une fiche de frais existe
     * 
     * @param String $idVisiteur ID du visiteur
     * @param String $mois       Mois sous la forme aaaamm
     * 
     * @return boolean vrai si la fiche existe, faux sinon
     */
    public function existeFicheFrais($idVisiteur, $mois) {
        ...
        $requetePrepare->bindParam(':unMois', $mois, PDO::PARAM_STR);
        $requetePrepare->execute();
        $nbFichesFrais = $requetePrepare->fetch();
        return $nbFichesFrais['nbfiches'];
    }
